...
// -- Delete
 ...
 // Initialization: só Dashboard, demais serão “lazy-loaded”// dashboard.js
...
async function adminFetch(url, opts = {}) {
  ...
  formRule.reset();
   ...
   formRule['rule-id'].disabled = true;
   if (rule) {
     // nome mantido (definido em openRuleModal)
   } else {
     // nome mantido (definido em openRuleModal)
   }
   if (rule) {
     formRule['rule-type'].value    = rule.type;
     // pattern mantido (definido em openRuleModal)
     // reply mantido (definido em openRuleModal)
   }
   setRuleFormFromRule(rule);
   modalRule.classList.remove('hidden');
  }
...
// --- Limpeza total do formulário de Regra ---
function clearRuleForm() {
  ...
  if (formRule['rule-nome'])  // nome mantido (definido em openRuleModal)
  if (formRule['rule-type'])  formRule['rule-type'].value = 'message';
  if (formRule['rule-pattern']) // pattern mantido (definido em openRuleModal)
  if (formRule['rule-reply']) // reply mantido (definido em openRuleModal)
  // limpa editor Quill
  try {
    ...
    window.rules = regras;
      cont.querySelectorAll('.btn-edit-regra').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          if (r) openRuleModal(JSON.parse(JSON.stringify(r)));
        });
      });
      cont.querySelectorAll('.btn-delete-regra').forEach(btn => {
        btn.addEventListener('click', async () => {
