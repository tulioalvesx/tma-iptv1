...
async function adminFetch(url, opts = {}) {
  ...
  // sincroniza editor rich-text (Quill) com o hidden
  try {
    var qEl = document.querySelector('#response-editor .ql-editor');
   ...
   if (formRule['rule-reply']) formRule['rule-reply'].value = '';
   try { var qEl = document.querySelector('#response-editor .ql-editor'); if (qEl) qEl.innerHTML = ''; } catch {}
   }
   if (rule) {
     formRule['rule-type'].value    = rule.type;
     formRule['rule-pattern'].value = rule.pattern;
     formRule['rule-reply'].value   = (typeof rule.reply === 'string') ? rule.reply : '';
     try { var qEl = document.querySelector('#response-editor .ql-editor'); if (qEl) qEl.innerHTML = formRule['rule-reply'].value; } catch {}
   }
   setRuleFormFromRule(rule);
   modalRule.classList.remove('hidden');
  }
...
formRule.addEventListener('submit', async (e) => {
  // copia HTML do editor Quill para o input hidden
  try { var qEl = document.querySelector('#response-editor .ql-editor'); if (qEl && formRule['rule-reply']) formRule['rule-reply'].value = qEl.innerHTML.trim(); } catch {}
